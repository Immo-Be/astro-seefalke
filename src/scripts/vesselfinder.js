export default function () {
  const myDiv = document.getElementById('vesselfinder');
  if (!myDiv) return;
  ...
  var obj =
    window.map_options_name &&
    window.map_options_name !== '' &&
    isObject(window[window.map_options_name])
  ...
  if (rh.indexOf('file://') >= 0 || rh.indexOf('file%3A%2F%2F') >= 0)
    rh = 'testingonly';
  ...
  var fn =
    typeof obj.fleet_name === 'undefined'
      ? false
      : encodeURIComponent(obj.fleet_name);
  if (fn !== false && fn.indexOf('@') >= 0) {
    fn = false;
  }

  if (obj.store_position === undefined) {
    obj.store_position = true;
  } else {
    obj.store_position = parseBoolValue(obj.store_position);
  }
  if (obj.mmsi !== undefined && parseInt(obj.mmsi) != obj.mmsi) {
    obj.mmsi = undefined;
  }
  if (obj.imo !== undefined && parseInt(obj.imo) != obj.imo) {
    obj.imo = undefined;
  }
  if (obj.latitude !== undefined && parseFloat(obj.latitude) != obj.latitude) {
    obj.latitude = undefined;
  }
  if (
    obj.longitude !== undefined &&
    parseFloat(obj.longitude) != obj.longitude
  ) {
    obj.longitude = undefined;
  }
  myDiv.innerHTML =
    ...
    'zoom=' +
    (obj.zoom === undefined ? 'undefined' : obj.zoom) +
    (obj.latitude === undefined
      ? '&amp;lat=undefined'
      : '&amp;lat=' + obj.latitude) +
    (obj.longitude === undefined
      ...
      : '&amp;lon=' + obj.longitude) +
    '&amp;width=' +
    encodeURIComponent(obj.width) +
    '&amp;height=' +
    encodeURIComponent(obj.height) +
    '&amp;names=' +
    obj.names +
    ...
    (obj.fleet_name === undefined
      ? '&amp;fleet_name=false'
      : '&amp;fleet_name=' + fn) +
    (obj.fleet_timespan !== undefined
      ? '&amp;fleet_timespan=' + obj.fleet_timespan
      : '') +
    ...
    '&amp;clicktoact=' +
    obj.click_to_activate +
    '&amp;store_pos=' +
    obj.store_position +
    (obj.fil === undefined ? '' : '&amp;fil=' + obj.fil) +
    (obj.default_maptype === undefined
      ? ''
      : '&amp;default_maptype=' + obj.default_maptype) +
    '&amp;ra=' +
    rh +
